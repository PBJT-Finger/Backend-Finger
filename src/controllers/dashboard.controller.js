...
class DashboardController {
  ...
  static async getSummary(req, res) {
    try {
      ...
      // Get total employees
      const [totalDosen, totalKaryawan, totalDevices] = await Promise.all([
        prisma.employee.count({ where: { jabatan: 'DOSEN', is_active: true } }),
        prisma.employee.count({ where: { jabatan: 'KARYAWAN', is_active: true } }),
        prisma.device.count({ where: { is_active: true } })
      ]);

      stats.total = {
        employees: totalDosen + totalKaryawan,
        dosen: totalDosen,
      ...
      stats.monthly = {
        total_attendance: monthlyAttendance,
