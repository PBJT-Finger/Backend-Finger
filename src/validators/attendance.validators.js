...
const validateSummary = [
    query('startDate')
        ...
        .isISO8601().withMessage('startDate must be valid ISO date (YYYY-MM-DD)')
        .custom((value, { req }) => {
    ...
    query('endDate')
        ...
        .isISO8601().withMessage('endDate must be valid ISO date (YYYY-MM-DD)')
        .custom((value, { req }) => {
            const date = new Date(value);
            if (isNaN(date.getTime())) {
    ...
    query('jabatan')
        .optional()
        .isIn(['DOSEN', 'KARYAWAN']).withMessage('jabatan must be DOSEN or KARYAWAN'),
...
const validateAttendanceFilters = [
    ...
    query('tanggal_mulai')
        .optional()
        .isISO8601().withMessage('tanggal_mulai must be valid date (YYYY-MM-DD)'),

    query('tanggal_akhir')
        .optional()
        .isISO8601().withMessage('tanggal_akhir must be valid date (YYYY-MM-DD)'),

    query('tipe_absensi')
        .optional()
        .isIn(['MASUK', 'PULANG']).withMessage('tipe_absensi must be MASUK or PULANG')
];
...
const validateAttendanceId = [
    param('id')
        ...
        .isInt({ min: 1 }).withMessage('Attendance ID must be positive integer')
        .toInt()
];
...
const validateRekapParams = [
    query('bulan')
        .notEmpty().withMessage('bulan is required')
        .isInt({ min: 1, max: 12 }).withMessage('bulan must be between 1 and 12')
        .toInt(),

    query('tahun')
        .notEmpty().withMessage('tahun is required')
        .isInt({ min: 2020, max: 2100 }).withMessage('tahun must be between 2020 and 2100')
        .toInt()
];
