...
async function main() {
    console.log("Checking duplicates in recent attendance...");
    const att = await prisma.attendance.findMany({
        orderBy: { id: 'desc' },
        take: 50
    });

    console.log(`Fetched ${att.length} recent records.`);

    // Find dupes by same user_id and tanggal
    let dupeCount = 0;
    for (let i = 0; i < att.length; i++) {
        for (let j = i + 1; j < att.length; j++) {
            if (att[i].user_id === att[j].user_id && att[i].tanggal.getTime() === att[j].tanggal.getTime() && att[i].is_deleted === false && att[j].is_deleted === false) {
                console.log(`DUPLICATE FOUND for user: ${att[i].nama}`);
